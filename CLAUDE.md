...
| `docs/skirnir-probing-bench-checklist.md` | **DOC-11** hardware-in-the-loop bring-up for the host-side probing pipeline + rotary wizards (probe = GPIO21, `Pn:P`). Physical accuracy is compile/host-only until this is run on the board — companion to the homing/4th-axis/spindle checklists, and **depends on DOC-10 A-axis sign-off first**. Covers the typed `[PRB:]`/latch, the side-vs-top Z probe split, the operator-editable bench params, and the center-finder accuracy check against a dial indicator. |
