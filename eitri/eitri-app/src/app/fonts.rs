...
//!
//! Mirrors skirnir's font stack — **Roboto** as the proportional UI face and **JetBrains Mono** for the
//! coordinate readouts, the G-code view, and the log — so the two apps read as one family.
...
//! so the family heads are the Regular faces. The Medium/Bold faces are vendored too (Roboto 400/500/700 and
//! JetBrains Mono 400/500/700) and registered under their own named keys so a view can reach a
...
pub fn definitions() -> FontDefinitions {
  ...
  // Monospace: JetBrains Mono Regular first (the G-code/log face), Roboto Regular as a fallback, then defaults.
...
mod tests {
  ...
  /// family. This is what makes the G-code view and coordinate readouts render in JetBrains Mono and the rest
  ...
  fn roboto_heads_proportional_and_jetbrains_mono_heads_monospace() {
    ...
    assert_eq!(
      ...
      "JetBrains Mono must head Monospace (G-code/log tabular figures)"
    );
