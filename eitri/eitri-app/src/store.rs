//! Shared host-side persistence primitives for the eitri-app config stores: the per-user config directory and an
//! atomic file write. The [`crate::config`] (JSON) store persists under the OS config
//! dir and wants a crash-safe write, so the mechanics live here rather than inline in the store. Mirrored from skirnir.
//!
//! **[`config_dir`]** resolves the OS config dir (e.g. `~/Library/Application Support/eitri` on macOS,
//! `~/.config/eitri` on Linux) via [`directories::ProjectDirs`]. **[`atomic_write`]** writes to a
...
/// resolution ignores the qualifier/organisation, yielding `~/.config/eitri` (on macOS it is
/// `~/Library/Application Support/eitri`); any future eitri-app
/// store should reuse this so everything lands in one directory.
const APP_NAME: &str = "eitri";
...
/// Resolve the absolute path of eitri-app's per-user config directory (e.g. `~/Library/Application Support/eitri` on
/// macOS, `~/.config/eitri` on Linux), creating nothing. Returns [`StoreError::NoConfigDir`] when no per-user
...
mod tests {
  ...
  fn atomic_write_creates_parents_and_lands_the_bytes() {
    let dir = std::env::temp_dir().join(format!("eitri-store-write-{}", std::process::id()));
  ...
  fn atomic_write_leaves_no_temp_sibling_behind() {
    ...
    let dir = std::env::temp_dir().join(format!("eitri-store-temp-{}", std::process::id()));
  ...
  fn atomic_write_overwrites_an_existing_file() {
    ...
    let dir = std::env::temp_dir().join(format!("eitri-store-overwrite-{}", std::process::id()));
    let path = dir.join("config.json");
    let _ = std::fs::remove_dir_all(&dir);
