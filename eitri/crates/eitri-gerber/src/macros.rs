...
impl MacroDef {
  ...
  pub fn parse(name: &str, blocks: &[&str], line: usize) -> Result<MacroDef> {
    ...
    for raw in blocks {
      ...
      // The comment primitive (code 0) is `0` followed by free-form text with no comma. The lexer strips all
      // whitespace, so that text fuses onto the leading `0` (e.g. `0Rectanglewithroundedcorners`) and the field no
      // longer parses as a bare integer — recognise the comment by its leading `0` and discard it. A bare `0`
      // still parses to code 0 and is handled below; only codes have a leading zero, so this cannot shadow one.
      let code: u32 = match first.parse() {
        Ok(code) => code,
        Err(_) if first.starts_with('0') => continue,
        Err(_) => return Err(GerberError::Syntax { line, message: format!("malformed macro primitive '{block}'") }),
      };
      if code == 0 {
...
mod tests {
  ...
  #[test]
  fn comment_primitives_are_skipped() {
    // KiCad's RoundRect macro leads with `0 <text>` comment primitives; the lexer strips their whitespace, so they
    // reach us fused like `0Rectanglewithroundedcorners`. They must be discarded, not rejected as malformed.
    let def = MacroDef::parse(
      "RoundRect",
      &["0Rectanglewithroundedcorners", "0$1Roundingradius", "0Adda4cornerspolygon", "1,1,$1,0,0"],
      1,
    )
    .expect("parse");
    assert_eq!(def.body.len(), 1, "only the circle primitive should survive");
    let geom = def.resolve(&[4.0], &backend()).expect("resolve");
    assert!((geom.unsigned_area() - std::f64::consts::PI * 4.0).abs() < 0.05, "area {}", geom.unsigned_area());
  }

  #[test]
  fn genuinely_malformed_primitive_is_rejected() {
    // A non-comment first field that is not an integer is a real syntax error, not a comment to swallow.
    assert!(matches!(MacroDef::parse("BAD", &["4Garbage,1,2"], 1), Err(GerberError::Syntax { .. })));
  }
