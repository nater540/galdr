...
//! global API in the parent module is a thin wrapper around exactly one of these). The engine philosophy —
//! framework-agnostic, host-testable, panic-free — applies equally to i18n. Mirrored from skirnir's i18n layer.
...
impl Translator {
  /// Create an empty registry. `use_isolating` defaults to `false`: eitri-app is an LTR desktop app, so the
