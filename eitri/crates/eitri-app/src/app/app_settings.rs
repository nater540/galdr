...
//! mirrored from skirnir's dialog. The view is the same thin-render shape as every panel: it reads [`UiState`]
//! plus the loaded [`Config`] and pushes [`Intent`]s; the shell owns applying them (re-skinning the live
//! context, mutating the config, writing the file on the explicit Save).
//!
//! Theme editing follows the config's model (see [`crate::config::theme`]): built-ins live in code and are
//! read-only here; "Create from current" snapshots the ACTIVE resolved palette into a fully-keyed user
//! [`ThemeOverride`] so every colour picker edits a concrete value, and each picker change round-trips as an
//! [`Intent::UpsertTheme`] so the shell re-resolves and the window re-skins live.
...
pub fn window(ctx: &egui::Context, state: &mut UiState, config: &Config, dirty: bool, sink: &mut IntentSink) {
  ...
  // The explicit `.id()` keeps egui's remembered position keyed on a STABLE token: this dialog is where the
  // operator switches language, and a title-derived id would jump the window the instant a locale is picked.
  egui::Window::new(tr!("app-settings-title"))
...
/// The dialog body: the language/theme/scale rows, the create-theme row, the colour editor for a user theme,
/// and the explicit Save row. Split from [`window`] so tests can mount it directly in a harness `Ui`.
pub fn body(ui: &mut egui::Ui, state: &mut UiState, config: &Config, dirty: bool, sink: &mut IntentSink) {
  ...
  // buttons and combo boxes land on the same row height as the computed-margin text field beside them.
  ui.spacing_mut().button_padding.y = 3.0;

  egui::Grid::new("app-settings-general").num_columns(2).spacing([12.0, 8.0]).show(ui, |ui| {
    // Language: the global i18n registry is the source of truth for both the selection and the choices.
    ui.label(RichText::new(tr!("app-settings-language")).color(palette.text_dim));
    ...
    // Theme: built-ins first, then the user themes from the config (a user theme shadowing a built-in name
    // shows once, as the built-in slot — resolution already prefers the user entry).
    ui.label(RichText::new(tr!("app-settings-theme")).color(palette.text_dim));
    ...
    // Font scale: a bounded slider over the SAME range the shell clamps to, so egui's always-clamp slider can
    // never silently rewrite a valid hand-edited value the moment the dialog opens.
    ui.label(RichText::new(tr!("app-settings-font-scale")).color(palette.text_dim));
  ...
  ui.horizontal(|ui| {
    ...
    let field = egui::TextEdit::singleline(&mut state.theme_name_draft)
      .hint_text(tr!("app-settings-new-theme-hint"))
      .margin(Metrics::text_field_margin(body_row, 6))
      .desired_width(180.0);
    ui.add(field);
    let name = state.theme_name_draft.trim().to_string();
    let create = egui::Button::new(tr!("app-settings-create"));
    if ui.add_enabled(!name.is_empty(), create).on_hover_text(tr!("app-settings-create-hint")).clicked() {
  ...
  // The save row is ANCHORED AS AN INNER BOTTOM PANEL and the editor/hint region below FILLS the remainder,
  // so the dialog's content always expands to exactly the height the window is given — what makes a manual
  // window resize stick (egui snaps a resizable window back to its content's natural height otherwise).
  egui::Panel::bottom("app-settings-save")
    ...
    .show(ui, |ui| {
      ui.separator();
      // The explicit save boundary: nothing writes the operator-owned config.json implicitly.
      ui.horizontal(|ui| {
        let save = egui::Button::new(RichText::new(tr!("app-settings-save")).color(palette.text)).fill(palette.accent);
        if ui.add_enabled(dirty, save).on_hover_text(tr!("app-settings-save-hint")).clicked() {
  ...
  // The colour editor: only a USER theme is editable (built-ins live in code). Each picker edits a clone of
  // the active entry; any change this frame is pushed once as a whole-theme upsert.
  match config.appearance.themes.get(config.appearance.active_theme.as_str()) {
    Some(theme) => {
      let mut edited = theme.clone();
      let mut changed = false;
      egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
        // Fall back to the ACTIVE resolved palette for any token a sparse hand-edited theme leaves unset, so
        // its picker starts from the colour actually on screen.
        ...
        for (index, (name, slot)) in edited.color_entries_mut().into_iter().enumerate() {
          ...
          if this_group != group {
            ...
            ui.add_space(6.0);
            ui.label(
              RichText::new(tr!(group_display_key(this_group)).to_uppercase())
          ...
          ui.horizontal(|ui| {
            // Edit UNmultiplied sRGBA channels: the unmultiplied picker round-trips the [r,g,b,a] the operator
            // actually chose, so the config keeps the picked colour exactly (a premultiplied capture decays
            // translucent colours toward black on every round trip).
            let mut rgba = slot
    ...
    None => {
      ui.label(RichText::new(tr!("app-settings-builtin-hint")).size(11.0).color(palette.text_dim));
    }
  }
}

/// A human-readable name for a locale tag in the language picker. Known bundled locales get their native
/// names; an unrecognised tag (a dropped-in locale pack) shows as its tag, still selectable and unambiguous.
...
/// The i18n message key for a colour-editor group header, mapped from the stable identity string
/// [`group_for`] returns.
...
/// `color_entries_mut` in declaration order, which is already grouped, so the editor emits a header whenever
/// the group changes. The mapping is total — an unmatched token lands in "other" (a unit test keeps that empty).
pub(crate) fn group_for(token: &str) -> &'static str {
  if token.starts_with("accent") {
    ...
    "status"
  } else if token.starts_with("copper")
    || token.starts_with("toolpath_")
    || token.starts_with("grid_")
    || matches!(token, "drill" | "geometry" | "selection" | "origin")
  {
    "canvas"
  } else if matches!(
    token,
...
mod tests {
  ...
  fn every_palette_token_maps_to_a_named_editor_group() {
    ...
    // "other" header (add a palette token → extend `group_for` or its prefixes).
  ...
  fn language_display_names_cover_the_bundled_locales() {
    ...
    assert_eq!(language_display_name("de-DE"), "de-DE", "an unknown tag falls back to itself");
  }
}
