...
eitri-app bundles two open-source typefaces, compiled into the binary via `include_bytes!` (see
`src/app/fonts.rs`). They back the egui font stack: Roboto is the proportional UI face, JetBrains Mono is the
monospace face used for coordinate readouts, the G-code view, and the log (which want tabular
figures, which JetBrains Mono provides).
